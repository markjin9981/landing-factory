...
import { logVisit } from '../services/googleSheetService';
...
const LandingPage: React.FC<Props> = ({ previewConfig }) => {
  ...
  // 2. Secondary: Hardcoded Config (Production)
  // 3. Tertiary: LocalStorage Drafts (Local Testing/Preview)
  
  let rawConfig: LandingConfig | undefined = previewConfig;

  if (!rawConfig && id) {
      // 1. Try Hardcoded
      rawConfig = LANDING_CONFIGS[id];

      // 2. Try LocalStorage (Drafts)
      if (!rawConfig) {
          try {
