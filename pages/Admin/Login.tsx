...
const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setErrorMsg(result.message || 'Google 로그인 실패');
        setIsLoading(false);
      }
      // If success, Supabase will redirect to OAuth provider
    } catch (e: any) {
      setErrorMsg('Google 로그인 처리 중 오류 발생');
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-gray-100">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••"
                required
                className="w-full pl-10 pr-12 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
              />
              <button
                type="button"
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full bg-white border border-gray-300 text-gray-700 py-3 rounded-lg font-medium hover:bg-gray-50 transition disabled:opacity-50 flex items-center justify-center gap-3"
        >
        ...
        <p className="mt-8 text-xs text-center text-gray-400">
          Authorized Personnel Only • Powered by Supabase
        </p>
      </div>
    </div>
  );
